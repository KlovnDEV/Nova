WEB.route(
  ...
  async (request, response) => {
    ...
    if (grade < 0) {
        const res = await DB.query("DELETE FROM user_licenses WHERE identifier = ? AND role = ?", identifier, role);
        if (!WEB.affected(res)) return WEB.nothingUpdated(response);
        return response.send(res);
    }

    const res = await DB.query(
      "INSERT INTO user_licenses (identifier, role, grade) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE grade=?",
      identifier,
      role,
      grade,
      grade
