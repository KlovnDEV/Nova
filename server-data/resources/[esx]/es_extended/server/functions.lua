		...
		if not command.allowConsole and playerId == 0 then
			...
			if error then
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						ESX.Warn(('[es_extended] %s'):format(msg))
					else
						xPlayer.triggerEvent('chat:addMessage', {args = {'ERROR', msg}})
					end
				end, function(msg)
					if playerId == 0 then
						ESX.Info(('[es_extended] %s'):format(msg))
					else
						xPlayer.triggerEvent('chat:addMessage', {args = {'INFO', msg}})
					end
				end)
			end
		end
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb2)
		MySQL.Async.execute('UPDATE users SET accounts = @accounts, `group` = @group, loadout = @loadout, position = @position, inventory = @inventory WHERE identifier = @identifier', {
...
ESX.GetPlayerFromLicense = function(license)
	for k,v in pairs(ESX.Players) do
