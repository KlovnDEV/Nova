...
function onPlayerJoined(playerId)
	local license = tostring(playerId)
	...
	if license then
		if ESX.GetPlayerFromLicense(license) then
			DropPlayer(playerId, ('there was an error loading your character!\nError code: identifier-active-ingame\n\nThis error is caused by a player on this server who has the same identifier as you have. Make sure you are not playing on the same Rockstar account.\n\nYour Rockstar license: %s'):format(license))
		else
			MySQL.Async.fetchAll('SELECT identifier FROM users WHERE license = @license', {
				['@license'] = license
			}, function(result)
				if result and #result > 0 then
					loadESXPlayer(license, playerId, result[1].identifier)
				else
					local identifier = ESX.GetRandomString(8)

					-- custom checks implementation --
					CustomTables.Run(identifier)
					...
					MySQL.Async.execute('INSERT INTO users (accounts, identifier, license) VALUES (@accounts, @identifier, @license)', {
						...
						loadESXPlayer(license, playerId, identifier)
					end)
				end
			end)
		end
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
	deferrals.defer()
	local playerId, license = source, tostring(playerId)
	Citizen.Wait(100)
...
function loadESXPlayer(license, playerId, identifier)
	local tasks = {}
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, `group`, loadout, position, inventory FROM users WHERE license = @license', {
			['@license'] = license
		}, function(result)
			--local job, grade, jobObject, gradeObject = result[1].job, tostring(result[1].job_grade)
			local job, grade, jobObject, gradeObject = "unemployed", "0"
...
--			end
			...
			userData.job.id = 0 --jobObject.id
			userData.job.name = 'unemployed' --jobObject.name
			userData.job.label = 'unemployed' --jobObject.label

			userData.job.grade = 0 --tonumber(grade)
			userData.job.grade_name = 'none'--gradeObject.name
			userData.job.grade_label = 'none'--gradeObject.label
			userData.job.grade_salary = 0 --gradeObject.salary
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, license, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, userData.playerName, userData.coords)
