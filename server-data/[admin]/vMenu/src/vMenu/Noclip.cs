        ...
        private async Task NoClipHandler()
        {
            ...
            while (NoclipActive)
            {
                ...
                if (Game.CurrentInputMode == InputMode.MouseAndKeyboard && UpdateOnscreenKeyboard() != 0 && !Game.IsPaused)
                {
                    ...
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponUnarmed))
                    {
                        MovingSpeed = 0;// very slow
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponMelee))
                    {
                        MovingSpeed = 1;// slow
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponShotgun))
                    {
                        MovingSpeed = 2;// normal
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponHeavy))
                    {
                        MovingSpeed = 3;// fast
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponSpecial))
                    {
                        MovingSpeed = 4;// very fast
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponHandgun))
                    {
                        MovingSpeed = 5;// extreme 1
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponSmg))
                    {
                        MovingSpeed = 6;// extreme 2
                    }
                    if (Game.IsDisabledControlPressed(0, Control.SelectWeaponAutoRifle))
                    {
                        MovingSpeed = 7;// max
                    }
